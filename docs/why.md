...
`go-ruby-augeas/augeas` reimplements Ruby's `augeas` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-augeas/augeas` directly, with no Ruby runtime;
...
Because `augeas` is pure Go with cgo disabled, it also compiles to **WebAssembly**
